...
const Testimonial = () => {
  ...
  const API_KEY = "YOUR_GOOGLE_API_KEY"; // visible in frontend
    ...
    const fetchReviews = async () => {
      try {
        ...
        if (data?.result?.reviews) {
          const mapped = data.result.reviews.map((review, idx) => ({
            id: idx,
            name: review.author_name,
            role: `Rating: ${review.rating} ⭐`,
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto w-full">
        <div className="flex flex-col lg:flex-row gap-8 md:gap-12 lg:gap-16">
          {/* Left */}
          <div ref={contentRef} className="w-full lg:w-2/5 space-y-4 md:space-y-6">
            <h1 className="text-amber-500 text-xl sm:text-2xl md:text-3xl font-bold">
              Client Testimonials
            </h1>
            <p className="text-2xl sm:text-3xl md:text-4xl font-normal leading-tight">
              Hear from our <br />
          ...
          {/* Right */}
          <div ref={sliderRef} className="w-full lg:w-3/5 relative">
            <div className="relative h-[500px] overflow-hidden">
              {/* Prev */}
              <button
                onClick={prevSlide}
                className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-lg flex items-center justify-center text-amber-500 hover:bg-amber-500 hover:text-white transition-all"
              >
                <FaChevronLeft />
              </button>
                  ...
                  testimonials.map((t, index) => (
                    ...
                    >
                      <div className="h-full flex flex-col lg:flex-row gap-6">
                        <div className="w-full lg:w-2/5 bg-gray-100 rounded-xl shadow-md">
                          <img
                            src={t.image}
                            alt={t.name}
                        ...
                        <div className="w-full lg:w-3/5 p-6 bg-gray-50 rounded-xl shadow-md flex flex-col justify-center">
                          ...
                          <p className="text-amber-500">{t.role}</p>
                        </div>
                      </div>
                    </div>
                  ))
              ...
              <button
                onClick={nextSlide}
                className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-lg flex items-center justify-center text-amber-500 hover:bg-amber-500 hover:text-white transition-all"
              >
                <FaChevronRight />
              </button>
            </div>
